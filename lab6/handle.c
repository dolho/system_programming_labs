...
ssize_t make_http_headers(char * buf, size_t len, const char * f_type, size_t body_length) {
    ...
    int num = snprintf(buf, len, "Content-Length: %lu\r\nConnection: close\r\n"
                                 "Content-Type: %s\r\nDate: %s", body_length, mime_type, time_buf);
    if (num < 0) {
        return -1;
    }
    return cur_pos + num;
...
ssize_t make_http_response(char * buf, size_t len, int status, char * headers, char * body, size_t len_body) {
    ...
    char * r_phrase;
    switch (status) {
        case 200: r_phrase = "OK"; break;
        case 403: r_phrase = "Forbidden"; break;
        case 503: r_phrase = "Service Unavailable"; break;
        default:  r_phrase = "Not Found"; break;
    }
